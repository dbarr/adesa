...
IDSTRING(rcsid, "$Id: mccp.c,v 1.4 2003/02/09 18:48:34 dave Exp $");

extern bool write_to_descriptor args((DESCRIPTOR_DATA *d, char *txt, int length));

unsigned char       compress_start[] = { IAC, SB, COMPRESS2, IAC, SE, '\0' };
...
compressStart(DESCRIPTOR_DATA *desc)
{
    ...
    s = (z_stream *) getmem(sizeof(*s));
    desc->out_compress_buf = (unsigned char *) getmem(COMPRESS_BUF_SIZE);
...
bool
writeCompressed(DESCRIPTOR_DATA *desc, char *txt, int length)
{
