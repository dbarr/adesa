...
IDSTRING(rcsid, "$Id: social-edit.c,v 1.8 2003/08/30 04:59:17 dave Exp $");
...
load_social_table()
{
    ...
    if (!fp) {
        bug("Could not open " SOCIAL_FILE " for reading.");
        exit(1);
    }
...
save_social_table()
{
    ...
    if (!fp) {
        bug("Could not open " SOCIAL_FILE " for writing.");
        return;
    }
...
do_sedit(CHAR_DATA *ch, char *argument)
{
    ...
    if (!str_cmp(cmd, "delete")) {    /* Remove a social */
        ...
        sprintf(buf, "Social: %s\n\r"
            ...
            social_table[iSocial].others_found, social_table[iSocial].vict_found, social_table[iSocial].char_auto, social_table[iSocial].others_auto);
