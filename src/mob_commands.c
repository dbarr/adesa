...
IDSTRING(rcsid, "$Id: mob_commands.c,v 1.5 2003/08/30 05:02:07 dave Exp $");
...
do_mpasound(CHAR_DATA *ch, char *argument)
{
    ...
    if (argument[0] == '\0') {
        bugf("Mpasound - No argument from vnum %d.", ch->pIndexData->vnum);
        return;
    }
...
do_mpkill(CHAR_DATA *ch, char *argument)
{
    ...
    if (arg[0] == '\0') {
        bugf("MpKill - No argument from vnum %d.", ch->pIndexData->vnum);
        return;
    }

    if ((victim = get_char_room(ch, arg)) == NULL) {
        bugf("MpKill - Victim not in room from vnum %d.", ch->pIndexData->vnum);
        return;
    }

    if (victim == ch) {
        bugf("MpKill - Bad victim to attack from vnum %d.", ch->pIndexData->vnum);
        return;
    }

    if (IS_AFFECTED(ch, AFF_CHARM) && ch->master == victim) {
        bugf("MpKill - Charmed mob attacking master from vnum %d.", ch->pIndexData->vnum);
        return;
    }

    if (ch->position == POS_FIGHTING) {
        bugf("MpKill - Already fighting from vnum %d", ch->pIndexData->vnum);
        return;
    }
...
do_mpjunk(CHAR_DATA *ch, char *argument)
{
    ...
    if (arg[0] == '\0') {
        bugf("Mpjunk - No argument from vnum %d.", ch->pIndexData->vnum);
        return;
    }
...
do_mpechoaround(CHAR_DATA *ch, char *argument)
{
    ...
    if (arg[0] == '\0') {
        bugf("Mpechoaround - No argument from vnum %d.", ch->pIndexData->vnum);
        return;
    }

    if (!(victim = get_char_room(ch, arg))) {
        bugf("Mpechoaround - Victim does not exist from vnum %d.", ch->pIndexData->vnum);
        return;
    }
...
do_mpechoat(CHAR_DATA *ch, char *argument)
{
    ...
    if (arg[0] == '\0' || argument[0] == '\0') {
        bugf("Mpechoat - No argument from vnum %d.", ch->pIndexData->vnum);
        return;
    }

    if (!(victim = get_char_room(ch, arg))) {
        bugf("Mpechoat - Victim does not exist from vnum %d.", ch->pIndexData->vnum);
        return;
    }
...
do_mpecho(CHAR_DATA *ch, char *argument)
{
    ...
    if (argument[0] == '\0') {
        bugf("Mpecho - Called w/o argument from vnum %d.", ch->pIndexData->vnum);
        return;
    }
...
do_mpmload(CHAR_DATA *ch, char *argument)
{
    ...
    if (arg[0] == '\0' || !is_number(arg)) {
        bugf("Mpmload - Bad vnum as arg from vnum %d.", ch->pIndexData->vnum);
        return;
    }

    if ((pMobIndex = get_mob_index(atoi(arg))) == NULL) {
        bugf("Mpmload - Bad mob vnum from vnum %d.", ch->pIndexData->vnum);
        return;
    }
...
do_mpoload(CHAR_DATA *ch, char *argument)
{
    ...
    if (arg1[0] == '\0' || !is_number(arg1)) {
        bugf("Mpoload - Bad syntax from vnum %d.", ch->pIndexData->vnum);
        return;
    }
    ...
    if (arg2[0] == '\0') {
        ...
        if (level < 0 || level > get_trust(ch)) {
            bugf("Mpoload - Bad level from vnum %d.", ch->pIndexData->vnum);
            return;
        }
    }

    if ((pObjIndex = get_obj_index(atoi(arg1))) == NULL) {
        bugf("Mpoload - Bad vnum arg from vnum %d.", ch->pIndexData->vnum);
        return;
    }
...
do_mppurge(CHAR_DATA *ch, char *argument)
{
    ...
    if ((victim = get_char_room(ch, arg)) == NULL) {
        if ((obj = get_obj_here(ch, arg))) {
            ...
            bugf("Mppurge - Bad argument from vnum %d.", ch->pIndexData->vnum);
        }
        return;
    }

    if (!IS_NPC(victim)) {
        bugf("Mppurge - Purging a PC from vnum %d.", ch->pIndexData->vnum);
        return;
    }
...
do_mpgoto(CHAR_DATA *ch, char *argument)
{
    ...
    one_argument(argument, arg);
    if (arg[0] == '\0') {
        bugf("Mpgoto - No argument from vnum %d.", ch->pIndexData->vnum);
        return;
    }

    if ((location = find_location(ch, arg)) == NULL) {
        bugf("Mpgoto - No such location from vnum %d.", ch->pIndexData->vnum);
        return;
    }
...
do_mpat(CHAR_DATA *ch, char *argument)
{
    ...
    if (arg[0] == '\0' || argument[0] == '\0') {
        bugf("Mpat - Bad argument from vnum %d.", ch->pIndexData->vnum);
        return;
    }

    if ((location = find_location(ch, arg)) == NULL) {
        bugf("Mpat - No such location from vnum %d.", ch->pIndexData->vnum);
        return;
    }
...
do_mptransfer(CHAR_DATA *ch, char *argument)
{
    ...
    if (arg1[0] == '\0') {
        bugf("Mptransfer - Bad syntax from vnum %d.", ch->pIndexData->vnum);
        return;
    }
    ...
    if (arg2[0] == '\0') {
        ...
        if ((location = find_location(ch, arg2)) == NULL) {
            bugf("Mptransfer - No such location from vnum %d.", ch->pIndexData->vnum);
            return;
        }

        if (room_is_private(location)) {
            bugf("Mptransfer - Private room from vnum %d.", ch->pIndexData->vnum);
            return;
        }
    }

    if ((victim = get_char_world(ch, arg1)) == NULL) {
        bugf("Mptransfer - No such person from vnum %d.", ch->pIndexData->vnum);
        return;
    }

    if (victim->in_room == NULL) {
        bugf("Mptransfer - Victim in Limbo from vnum %d.", ch->pIndexData->vnum);
        return;
    }
...
do_mpforce(CHAR_DATA *ch, char *argument)
{
    ...
    if (arg[0] == '\0' || argument[0] == '\0') {
        bugf("Mpforce - Bad syntax from vnum %d.", ch->pIndexData->vnum);
        return;
    }
    ...
    if (!str_cmp(arg, "all")) {
        ...
        if ((victim = get_char_room(ch, arg)) == NULL) {
            bugf("Mpforce - No such victim from vnum %d.", ch->pIndexData->vnum);
            return;
        }

        if (victim == ch) {
            bugf("Mpforce - Forcing oneself from vnum %d.", ch->pIndexData->vnum);
            return;
        }
...
void
do_mptongue(CHAR_DATA *ch, char *argument)
{
    char                arg[MAX_INPUT_LENGTH];
    char                buf[MAX_STRING_LENGTH];
    ...
    for (rch = ch->in_room->first_person; rch; rch = rch->next_in_room) {
        ...
        if (rch != ch)
            act((race == rch->race || rch->level >= LEVEL_HERO) ? buf2 : buf3, ch, NULL, rch, TO_VICT);
    }
